...
import { Check, Code, Brain, Server } from 'lucide-react';
...
const services: Service[] = [
  {
    ...
    features: ['React.js & HTML/CSS', 'Node.js (Express)', 'Laravel', 'FastAPI'],
  },
  {
    title: 'Emerging Tech',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-xl text-slate-400 max-w-2xl mx-auto">
            These are the areas I am most passionate about and where I have concentrated my academic and personal projects.
          </p>
        </div>

        {/* Increased gap and container width for "larger" feel */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto">
          {services.map((service, index) => (
            <div 
              key={service.title} 
              className="group relative rounded-3xl p-10 flex flex-col h-full bg-slate-900 border-2 border-brand-500 hover:border-brand-400 transition-all duration-300 hover:shadow-[0_0_40px_rgba(139,92,246,0.2)] hover:-translate-y-2 overflow-hidden"
            >
              {/* Decorative background glow */}
              <div className="absolute top-0 right-0 -mr-20 -mt-20 w-64 h-64 bg-brand-500/10 rounded-full blur-3xl group-hover:bg-brand-500/20 transition-all"></div>
              ...
              <div className="relative z-10 mb-6">
                  <div className="w-16 h-16 rounded-2xl bg-slate-800 border border-slate-700 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                    {index === 0 && <Server className="h-8 w-8 text-brand-400" />}
                    {index === 1 && <Brain className="h-8 w-8 text-brand-400" />}
                  </div>

                  <h3 className="text-2xl font-bold text-white mb-3">{service.title}</h3>
                  <p className="text-slate-300 text-base leading-relaxed min-h-[50px]">{service.description}</p>
              </div>
